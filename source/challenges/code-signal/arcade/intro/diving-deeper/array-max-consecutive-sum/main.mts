...
Given an array of numbers, this will find the largest
	sum of two (or more) adjacent items in the array.
...
**/
const findMaxRollingSum = (
	numbers: Array<number>,
	sliceSize: number = 2,
) => {
	// Determine the largest rolling sum.
	let maxSum = 0
	let startIndex = 0

	while (startIndex < numbers.length - sliceSize + 1) {
		// Determine the current rolling sum.
		let sum = 0
		let sliceIndex = 0
		...
		// Record this sum, if it is largest.
...
export default findMaxRollingSum
